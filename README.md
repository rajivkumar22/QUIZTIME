...
- Enter any topic and get instant quiz questions **generated by OpenAI's ChatGPT**
...
This app uses a **client-server architecture** to properly handle OpenAI API integration:

- **Frontend**: React.js app (runs on http://localhost:5176)
- **Backend**: Node.js Express server (runs on http://localhost:3001)
- **AI Integration**: Backend securely calls OpenAI API to avoid CORS issues
...
4. Click "Start Quiz" - the app will call OpenAI to generate custom questions
...
3. Backend calls OpenAI API with your API key
4. OpenAI generates custom questions about the topic
...
2. **Add Billing**: Ensure you have credits/billing set up
...
├── backend/
│   ├── server.js            # Express server for OpenAI API
